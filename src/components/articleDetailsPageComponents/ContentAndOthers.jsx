...
const ContentAndOthers = ({ singleArticle }) => {
  ...
  const publishDate = publicationDate
    ? format(new Date(publicationDate), "do MMM yyyy")
    : "Unknown";
  return (
    <>
      <div
        ...
        className="mb-4 md:mb-8 lg:mb-10 bg-base-200 py-3 sm:py-4 px-2 md:px-6 shadow-2xl shadow-neutral-600 rounded-2xl"
      >
        <div>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between lg:px-20 md:px-12 sm:px-4">
          ...
          <div>
            {tags?.map((tag, i) => (
              <span key={i} className="badge badge-secondary mr-2 text-[10px]">
                {tag}
              </span>
            ))}
          </div>
